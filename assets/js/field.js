/* ═══════════════════════════════════════════════════════════
   field.js — helperِ مشترکِ کامپوننتِ فیلد (.field)
   مدیریتِ حالات: idle / focus / error / success / loading / disabled
   استفاده:
     Field.set(input, 'error',   'قالبِ ایمیل نامعتبر است');
     Field.set(input, 'success', 'درست است');
     Field.set(input, 'loading');          // در حالِ بررسی…
     Field.set(input, 'disabled');
     Field.clear(input);                    // → idle
   `input` می‌تواند خودِ المنت یا id آن (رشته) باشد.
   ═══════════════════════════════════════════════════════════ */
(function () {
  'use strict';

  // آیکونِ داخلِ کادر (۲۰px)
  const ICONS = {
    error:
  ...
  /* ست‌کردنِ حالتِ یک فیلد + پیام/آیکون */
  function set(input, state, message) {
    ...
    // پیامِ زیرِ فیلد (فقط برای error/success)
  ...
  /* اتصالِ خودکارِ focus/blur: فقط وقتی فیلد در حالتِ idle/focus است،
     تا verdictِ error/success/loading/disabled بازنویسی نشود. */
  function bind(scope) {
