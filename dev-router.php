<?php
/**
 * dev-router.php — فقط برای سرور توسعه محلی (php -S) استفاده می‌شود.
 *
 * چون `php -S` فایل‌های .htaccess را نادیده می‌گیرد، این روتر همان رفتار
 * «URLهای تمیز بدون .php» را که در .htaccess هاست تعریف شده، شبیه‌سازی می‌کند:
 *   - /            → index.php
...
// و با ?page= برای صفحات). ریدایرکت 301 این درخواست‌ها را خراب می‌کند
// (POST→GET، حذف بدنه، و افتادن کوئری) و باعث «خطا در ارتباط با سرور» می‌شود.
...
// ۳۰۱: foo.php → /foo  (هماهنگ با .htaccess هاست)
if (preg_match('#^([^/]+)\.php$#', $path, $m)) {
    header('Location: /' . $m[1], true, 301);
    exit;
}

// فایل‌های واقعی موجود (css/js/woff/تصویر/...) را php -S مستقیم سرو کند
$full = $root . '/' . $path;
...
// بدون پسوند → نسخه .php اگر وجود داشته باشد
if (is_file($root . '/' . $path . '.php')) {
    require $root . '/' . $path . '.php';
    return true;
}

// در غیر این صورت بگذار php -S تصمیم بگیرد (۴۰۴ یا فایل استاتیک)
return false;
